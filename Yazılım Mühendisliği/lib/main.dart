      ...
      body: Center(
        child: ConstrainedBox(
          ...
          child: ListView.builder(
            ...
            // RoomServiceCategoryScreen içindeki itemBuilder kısmını bul ve burayla değiştir:
            itemBuilder: (context, index) {
  ...
  Widget build(BuildContext context) {
    final Map<String, List<Map<String, dynamic>>> menuData = {
      ...
      'Atıştırmalıklar ve Başlangıçlar': [
        ...
        // Diğer atıştırmalıklar için de benzer şekilde ekleme yapabilirsin...
      ],
      'Ana Yemekler': [
        {
                            ...
                            children: [
                              ...
                              _ActionItem(
                                  ...
                                  onTap: () => Navigator.push(
                                      context,
                                      MaterialPageRoute(builder: (context) => HousekeepingScreen(roomNumber: res.roomNumber))
                                  )
...
class _AdminLoginScreenState extends State<AdminLoginScreen> {
  ...
  void _login() {
    if (_emailController.text == 'admin@admin.com' && _passwordController.text == 'admin123') {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Admin girişi başarılı!'), backgroundColor: Colors.green));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Hatalı e-posta veya şifre!'), backgroundColor: Colors.red));
    }
  }
...
class HousekeepingScreen extends StatelessWidget {
  final String roomNumber;
  const HousekeepingScreen({super.key, required this.roomNumber});
            ...
            children: supplies.entries.map((category) {
              return Card(
                ...
                child: ExpansionTile(
                  ...
                  // ExpansionTile içindeki category.value.map kısmını şu şekilde güncelle:
                  children: category.value.map((item) => ListTile(
                    ...
                    leading: Container(
                      ...
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(8),
                        image: DecorationImage(
                          image: NetworkImage(item['img']!), // Resim URL'sini buradan alıyoruz
                          fit: BoxFit.cover,
                        ),
                      ),
                    ),
                    title: Text(item['name']!, style: const TextStyle(fontWeight: FontWeight.w500)), // Ürün ismini alıyoruz
                    trailing: ElevatedButton(
                      onPressed: () {
                        ScaffoldMessenger.of(context).showSnackBar(
